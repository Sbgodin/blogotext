...
function creer_fichier_zip($dossiers) {
	$zipfile = $GLOBALS['BT_ROOT_PATH'].$GLOBALS['dossier_backup'].'/'.'archive_site.zip';
	$zip = new ZipArchive;
				...
				// s’ils n’y sont pas, on l’ajoute en prennant comme bt_id la date de création du fichier
					...
					foreach ($images_sur_disque as $i => $image) {
						...
						if (!in_array($image, $liste_images)) {
							$time = filemtime($GLOBALS['BT_ROOT_PATH'].$GLOBALS['dossier_images'].'/'.$image);
							$id = date('YmdHis', $time);
							...
							$new_img = array(
								'bt_id' => $id,
								'bt_type' => 'image',
								'bt_fileext' => strtolower(pathinfo($GLOBALS['BT_ROOT_PATH'].$GLOBALS['dossier_images'].'/'.$image, PATHINFO_EXTENSION)),
								'bt_filesize' => filesize($GLOBALS['BT_ROOT_PATH'].$GLOBALS['dossier_images'].'/'.$image),
								'bt_filename' => $image,
								'bt_content' => '',
								'bt_wiki_content' => '',
								'bt_checksum' => sha1_file($GLOBALS['BT_ROOT_PATH'].$GLOBALS['dossier_images'].'/'.$image),
								'bt_statut' => 0,
							);
							// l’ajoute au tableau
