...
$comment['version'] = parse_xml($file, 'bt_version');
		...
		if ($comment['auteur'] == $GLOBALS['auteur']) $comment['auteur'] = '<span class="admin">'.$comment['auteur'].'</span>';
		if ($comment['webpage'] != '') $comment['auteur'] = '<a href="'.$comment['webpage'].'" class="webpage">'.$comment['auteur'].'</a>';
		$comment['anchor'] = '<a href="#form-commentaire" onclick="reply(\'@['.$comment['auteur_ss_lien'].'|#'.article_anchor($comment['id']).'] : \'); ">@</a> <a href="#'.article_anchor($comment['id']).'" id="'.article_anchor($comment['id']).'">#</a>';
